...
def _grounded_fallback(registry: ToolRegistry) -> str:
    ...
    parts = [
        ...
        '예: {"project_hint":"JARVIS", "tasks":[{"title":"작업 제목", "reason":"작업 이유"}]}',
    ...
    if registry.get("propose_next_action") is not None and registry.get("propose_next_action").handler is None:
        parts.append("다음 행동은 현재 읽은 문맥을 바탕으로 대화에서 제안할 수 있지만, 별도 자동 기능으로 가장하지 않습니다.")
    return "\n\n".join(parts)
...
def _user_facing_content(
    ...
    leaked = leaked or any(phrase.strip() in result for phrase in internal_phrases)
